...
const PartnerPage = () => {
      ...
      try {
        ...
        if (!storedUserData || userDataLocation.username) {
          ...
          const res = await axios.get(
            `${API_URL}/get_user/${userDataLocation.username}`,
            {
              headers: {
  ...
  if (error) {
    return (
      <div>
        <HeaderComponent />
        <div className={stylePreloader.error}>
          Ошибка: {error}. Ваша ссесия закончилась. Нажмите кнопку "выход"
        </div>
        ;
      </div>
    );
    ...
    <div>
      ...
      <div className={styles.partner}>
        <p className={styles.hello_partner}>
          {userData.name}, добро пожаловать!
        </p>
        <div className={styles.link_container}>
          <Link to="/create_build" className={styles.partner_link}>
            Добавить новый объект
          </Link>
          <Link to="/create_user" className={styles.partner_link}>
            Добавить сотрудника
          </Link>
          <Link to="/create_client" className={styles.partner_link}>
            Добавить клиента
          </Link>
          <Link to="/all_builds" className={styles.partner_link}>
        ...
        <div className={styles.left_block}>
          <p className={styles.hello_partner}>
            {userData.name}, Ваши клиенты RealtyHub.ME :
          </p>
          <ClientsList
            clientList={clients}
        ...
        <div className={styles.right_block}>
          <p className={styles.hello_partner}>
            {userData.name}, Ваши объекты RealtyHub.ME :
          </p>
